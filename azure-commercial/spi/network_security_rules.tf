...
resource "azurerm_network_security_rule" "AllowVnet3389" {
  ...
  destination_address_prefixes = [var.subnet_address_space_app1,"199.207.253.94","203.199.72.210"]
  resource_group_name          = azurerm_resource_group.cospivnetrg.name
  network_security_group_name  = azurerm_network_security_group.spiapp1nsg.name
}
...
resource "azurerm_network_security_rule" "spiagw1_allow_port_443_from_AZ_FW_PIPs" {
  name                        = "allow_port_443_from_AZ_FW_PIPs"
  priority                    = 180
  direction                   = "Inbound"
  access                      = "Allow"
  description                 = "To access SPI URLs behind AZ FW"
  protocol                    = "TCP"
  source_port_range           = "*"
  destination_port_range      = "443"
...
resource "azurerm_network_security_rule" "Inbound_Allow_443_From_ASE" {
  name                        = "Inbound_Allow_443_From_ASE"
  description                 = "Allows all traffic from ASE"
  priority                    = 168
  direction                   = "Inbound"
  access                      = "Allow"
  protocol                    = "*"
