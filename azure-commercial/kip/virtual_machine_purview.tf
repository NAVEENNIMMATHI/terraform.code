module "purview1vm" {
  ...
  count_value                  = var.count_value_pvw1vm
  vm_size                      = var.vm_size_pvw1vm
  ...
  data_disk_size               = var.data_disk_size_pvw1vm
  ...
  admin_username = lower(
    "${var.application_acronym}PVW1${var.environment == "production" ? "PD1" : var.environment_acronym}ADMIN",
  )
