  ...
  template_body = <<DEPLOY
{
  ...
  "parameters": {
    ...
    "PaloInboundIPAddress": {
      "type": "string",
      "defaultValue": "PaloInboundIPAddress",
      "metadata": {
        "description": "PaloInboundIPAddress"
      }
    },
    "location": {
      "type": "string",
...
{"name": "OutboundTraffictoPaloAlto", "properties": { "addressprefix": "0.0.0.0/0", "nextHopType": "VirtualAppliance", "nextHopIpAddress": "[parameters('PaloInboundIPAddress')]" }}
...
DEPLOY
  ...
  parameters = {
    ...
    "routeTableToAzureService"    = azurerm_route_table.miroutetable.name
    "nsgForManagedInstanceSubnet" = module.db1nsg.network_security_group_name
    "PaloInboundIPAddress"        = var.PaloInboundIPAddress
    "location"                    = module.applicationrg.resource_group_location
    "managedInstanceName"         = var.environment == "production" ? "copdeuskipmi" : "smi-${var.location_acronym}-${var.application_acronym}-${var.environment_acronym}"
    ...
    "subnetId"                    = azurerm_subnet.db1subnet.id
  ...
  depends_on = [
    module.db1nsg,
    azurerm_subnet.db1subnet,
    module.db1_subnet_nsg_association,
    azurerm_route_table.miroutetable,
    azurerm_subnet_route_table_association.kipdb1_subnet,
