...
module "mssql_server_kpiq" {
    ...
    sqlTostorage_assignment_name_guiid  = var.sql_assignment_guid

    tags                                = var.tags
}

/*
resource "azurerm_mssql_elasticpool" "kpiqelasticpool" {
  name                         = "${var.application_acronym}elasticpool${var.environment_acronym}"
  location                     = module.applicationrg.resource_group_location
  resource_group_name          = module.applicationrg.resource_group_name
  server_name                  = module.mssql_server_kpiq.sql_server_name
