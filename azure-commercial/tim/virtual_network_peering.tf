...
resource "azurerm_virtual_network_peering" "coretovnet" {
  ...
  remote_virtual_network_id    = "/subscriptions/${data.azurerm_subscription.current.subscription_id}/resourceGroups/${module.timrg.resource_group_name}/providers/Microsoft.Network/virtualNetworks/${module.applicationvnet.virtual_network_name}"
  allow_virtual_network_access = true
  allow_forwarded_traffic      = true
