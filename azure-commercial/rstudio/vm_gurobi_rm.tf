resource "azurerm_network_interface" "rstudio_gurobi_rm_nic" {
  name                          = var.environment == "production" ? "UEMCRSTUDGRBRM1P1-nic" : lower("nic-${var.location_acronym}-${var.application_acronym}-${var.rstudio_gurobirmvm_name_prefix1}-${var.environment_acronym}")
  ...
  ip_configuration {
    name                          = var.environment == "production" ? "UEMCRSTUDGRBRM1P1-config" : upper("VML${var.location_acronym}${var.application_acronym}${var.rstudio_gurobirmvm_name_prefix1}${var.environment_acronym}-config")
...
resource "azurerm_virtual_machine" "rstudiogurobivmrminstance" {
  name                             = var.environment == "production" ? "UEMCRSTUDGRBRM1P1" : upper("VML${var.location_acronym}${var.application_acronym}${var.rstudio_gurobirmvm_name_prefix1}${var.environment_acronym}")
  location                         = module.applicationrg.resource_group_location
  resource_group_name              = module.applicationrg.resource_group_name
  network_interface_ids            = ["${element(azurerm_network_interface.rstudio_gurobi_rm_nic.*.id, count.index)}"]
  vm_size                          = var.rstudio_gurobirmvm_size
  count                            = var.rstudio_gurobivm_count
  availability_set_id              = module.availabilityset_gurobirm.availability_set_id
  ...
  storage_os_disk {
    name              = var.environment == "production" ? "UEMCRSTUDGRBRM1P1_OSDisk" : "${upper("VML${var.location_acronym}${var.application_acronym}${var.rstudio_gurobirmvm_name_prefix1}${var.environment_acronym}")}_OSDisk"
    caching           = "ReadWrite"
  ...
  storage_data_disk {
    name              = var.environment == "production" ? "UEMCRSTUDGRBRM1P1_DataDisk_1" : "${upper("VML${var.location_acronym}${var.application_acronym}${var.rstudio_gurobirmvm_name_prefix1}${var.environment_acronym}")}_DataDisk_1"
    managed_disk_type = "Premium_LRS"
  ...
  storage_data_disk {
    name              = var.environment == "production" ? "UEMCRSTUDGRBRM1P1_LogDisk_1" : "${upper("VML${var.location_acronym}${var.application_acronym}${var.rstudio_gurobirmvm_name_prefix1}${var.environment_acronym}")}_LogDisk_1"
    managed_disk_type = "Premium_LRS"
    create_option     = var.target_deployment_environment == "prod" ? "Attach" : "Empty"
    lun               = 1
    disk_size_gb      = "128"
  ...
  os_profile {
    computer_name  = var.environment == "production" ? "UEMCRSTUDGRBRM1P1" : upper("VML${var.location_acronym}${var.application_acronym}${var.rstudio_gurobirmvm_name_prefix1}${var.environment_acronym}")
...
resource "azurerm_network_interface" "rstudio_gurobi2_rm_nic" {  
  name                          = lower("nic-${var.location_acronym}-${var.application_acronym}-GRM2-${var.environment_acronym}")
  resource_group_name           = module.applicationrg.resource_group_name
  location                      = module.applicationrg.resource_group_location  
  enable_accelerated_networking = true

  ip_configuration {
    name                          = upper("VML${var.location_acronym}${var.application_acronym}GRM2${var.environment_acronym}-config")
    subnet_id                     = module.gurobi_subnet.subnet_id
    private_ip_address_allocation = "dynamic"
  }
  tags = var.tags
...
resource "azurerm_linux_virtual_machine" "rstudiogurobivmrminstance" {
  depends_on            = [module.availabilityset_gurobirm]
  availability_set_id   = module.availabilityset_gurobirm.availability_set_id
  name                  = upper("VML${var.location_acronym}${var.application_acronym}GRM2${var.environment_acronym}")
  location              = module.applicationrg.resource_group_location
  resource_group_name   = module.applicationrg.resource_group_name
  network_interface_ids = [ azurerm_network_interface.rstudio_gurobi2_rm_nic.id ]
  ...
  os_disk {
    name                  = "${upper("VML${var.location_acronym}${var.application_acronym}GRM2${var.environment_acronym}")}_osDisk"
    caching               = "ReadWrite"
  ...
  admin_password  = var.rstudio_gurobivm_admin_password
  computer_name   = upper("VML${var.location_acronym}${var.application_acronym}GRM2${var.environment_acronym}")
...
resource "azurerm_managed_disk" "managed_disk_rstudiogurobivmrminstance_data_1" {
  depends_on           = [azurerm_linux_virtual_machine.rstudiogurobivmrminstance]
  name                 = "${upper("VML${var.location_acronym}${var.application_acronym}GRM2${var.environment_acronym}")}_DataDisk_1"
  location             = module.applicationrg.resource_group_location
  resource_group_name  = module.applicationrg.resource_group_name
  storage_account_type = "Premium_LRS"
  create_option        = "Empty"
...
resource "azurerm_virtual_machine_data_disk_attachment" "managed_disk_rstudiogurobivmrminstance_data_1" {
  depends_on         = [azurerm_managed_disk.managed_disk_rstudiogurobivmrminstance_data_1]
  managed_disk_id    = azurerm_managed_disk.managed_disk_rstudiogurobivmrminstance_data_1.id
  virtual_machine_id = azurerm_linux_virtual_machine.rstudiogurobivmrminstance.id
  lun                = 0
  caching            = "ReadWrite"
}

resource "azurerm_managed_disk" "managed_disk_rstudiogurobivmrminstance_log_1" {
  depends_on           = [azurerm_linux_virtual_machine.rstudiogurobivmrminstance]
  name                 = "${upper("VML${var.location_acronym}${var.application_acronym}GRM2${var.environment_acronym}")}_LogDisk_1"
  location             = module.applicationrg.resource_group_location
  resource_group_name  = module.applicationrg.resource_group_name
  storage_account_type = "Premium_LRS"
  create_option        = "Empty"
...
resource "azurerm_virtual_machine_data_disk_attachment" "managed_disk_rstudiogurobivmrminstance_log_1" {
  depends_on         = [azurerm_managed_disk.managed_disk_rstudiogurobivmrminstance_log_1]
  managed_disk_id    = azurerm_managed_disk.managed_disk_rstudiogurobivmrminstance_log_1.id
  virtual_machine_id = azurerm_linux_virtual_machine.rstudiogurobivmrminstance.id
  lun                = 1
  caching            = "ReadWrite"
}

resource "azurerm_virtual_machine_extension" "dependency_agent_linux_gurobi_rm" {
  depends_on                    = [azurerm_linux_virtual_machine.rstudiogurobivmrminstance]
...
resource "azurerm_virtual_machine_extension" "oms_agent_for_linux_gurobi_rm" {
  depends_on = [azurerm_virtual_machine_extension.dependency_agent_linux_gurobi_rm]  
...
#  depends_on = [azurerm_virtual_machine_extension.oms_agent_for_linux_gurobi_rm]
#  
#  name                          = "DatadogAgent"
#  virtual_machine_id            = azurerm_linux_virtual_machine.rstudiogurobivmrminstance.id
...
resource "null_resource" "gurobi_create_working_dir_rm" {
  depends_on = [azurerm_virtual_machine_extension.oms_agent_for_linux_gurobi_rm]
  triggers  = {
  ...
  connection {
    ...
    host     = azurerm_network_interface.rstudio_gurobi2_rm_nic.private_ip_address
    port     = 22
    timeout  = "10m"
  }
...
resource "null_resource" "gurobi_copy_scripts_dir_rm" {
  depends_on = [null_resource.gurobi_create_working_dir_rm]
  triggers  = {
    forced_iteration = uuid()
  }
...
resource "null_resource" "gurobi_configure_vm_disk_rm" {
  depends_on = [null_resource.gurobi_copy_scripts_dir_rm]
  triggers  = {
    forced_iteration = uuid()
  }
...
resource "null_resource" "gurobi_configure_vm_rm" {
  depends_on = [null_resource.gurobi_configure_vm_disk_rm]
  triggers  = {
    forced_iteration = uuid()
  }
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "vm_name=\"${azurerm_linux_virtual_machine.rstudiogurobivmrminstance.computer_name}\"",
      ...
      "mongodb_conn_string=\"${mongodbatlas_cluster.mongodb_atlas_cluster.connection_strings.0.private_endpoint.0.srv_connection_string}\"",
      "gurobi_manager_server=\"${azurerm_linux_virtual_machine.rstudiogurobivmrminstance.computer_name}.$domain_name\"",
    ...
    #"echo \"$provisionerLogPrefix Provisioner 'vm_common_rstudio_dependencies.sh' Starting \" >> $logFilePath",
      ...
      "echo \"$provisionerLogPrefix Provisioner 'vm_gurobi_rm_softwares.sh' Starting \" >> $logFilePath",
      "echo \"$sudoPassword\" | sudo -S $workingDir/vm_gurobi_rm_softwares.sh \"$logFilePath\" \"$rstudio_gurobi_package\" \"$artifactory_url\" \"$gurobi_license_file\" \"$mongodb_user\" \"$mongodb_user_password\" \"$mongodb_conn_string\"",
      "echo \"$provisionerLogPrefix Provisioner 'vm_gurobi_rm_softwares.sh' Complete \" >> $logFilePath",
