...
resource "azurerm_role_definition" "crisis_planning_data_scientists_role" {
  name        = "limited-storage-contributor-ds-${var.engagement_name}${var.separator}${var.environment}"
  scope       = module.ignite_stg.id
  description = "This is a custom role created for data scientist to Upload/download data to storage account."
...
resource "azurerm_role_definition" "crisis_planning_users_role" {
  name        = "limited-storage-contributor-users-${var.engagement_name}${var.separator}${var.environment}"
  scope       = module.ignite_stg.id
  description = "This is a custom role created for user Upload/download data to storage account."
