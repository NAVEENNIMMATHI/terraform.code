locals {
  rdgw1vm_name = (var.environment_acronym == "dv" || var.environment_acronym == "pd") ? "UEMCRDSGW1P1" : "VMW${upper(var.location_acronym)}RDSGW1${upper(var.environment_acronym)}"
}
resource "azurerm_network_interface" "rdgw1nic" {
  count = var.environment_acronym == "pd" ? 1 : 0

  name                = "${local.rdgw1vm_name}-nic"
  resource_group_name = azurerm_resource_group.rds.name
  location            = azurerm_resource_group.rds.location

  ip_configuration {
    name                          = "${local.rdgw1vm_name}-config"
    subnet_id                     = azurerm_subnet.rdgw.id
    private_ip_address_allocation = "Static"
    private_ip_address            = var.rdgw1_nic_private_ip_address
  }

  tags = var.tags
}

resource "azurerm_virtual_machine" "rdgw1vm" {
  count = var.environment_acronym == "pd" ? 1 : 0

  name                  = local.rdgw1vm_name
  location              = azurerm_resource_group.rds.location
  resource_group_name   = azurerm_resource_group.rds.name
  network_interface_ids = [element(azurerm_network_interface.rdgw1nic.*.id, count.index)]
  ...
  storage_os_disk {
    name              = "${local.rdgw1vm_name}_OSDisk"
    caching           = "ReadWrite"
    create_option     = "FromImage"
    managed_disk_type = var.environment == "production" ? "Premium_LRS" : "Standard_LRS"
  }

  storage_data_disk {
    name              = "${local.rdgw1vm_name}_DataDisk_1"
  ...
  storage_data_disk {
    name              = "${local.rdgw1vm_name}_DataDisk_2"
    managed_disk_type = "Standard_LRS"
    create_option     = "Empty"
    lun               = 1
    disk_size_gb      = "200"
  ...
  os_profile {
    computer_name  = local.rdgw1vm_name
    admin_username = var.rdgw1_admin_username
...
# resource "azurerm_virtual_machine_extension" "rdgw1vm_custom_ext_win_vm" {
#   count = var.environment_acronym == "dv" ? 1 : 0

#   name                       = "CustomScriptExtension"
#   virtual_machine_id         = element(azurerm_virtual_machine.rdgw1vm.*.id, count.index)
