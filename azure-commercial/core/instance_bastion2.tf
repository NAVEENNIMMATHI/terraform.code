locals {
  bastion2vm_name = (var.environment_acronym == "dv" || var.environment_acronym == "pd") ? "UEMCBASTION2P1" : "VMW${upper(var.location_acronym)}BSTN2${upper(var.environment_acronym)}"
}

resource "azurerm_network_interface" "bastion2nic" {
  count = (var.environment_acronym == "dv" || var.environment_acronym == "pd")  ? 1 : 0
  
  name                = "${local.bastion2vm_name}-nic"
  resource_group_name = azurerm_resource_group.bstnhosts.name
  location            = azurerm_resource_group.bstnhosts.location

  ip_configuration {
    name                          = "${local.bastion2vm_name}-config"
    subnet_id                     = azurerm_subnet.bastion1.id
    private_ip_address_allocation = "Static"
    private_ip_address            = var.bastion2_nic_private_ip_address
    public_ip_address_id          = var.environment == "production" ? azurerm_public_ip.bastion2pip.id : ""
    #public_ip_address_id          = azurerm_public_ip.bastion2pip.id
  }

  tags = var.tags
}

resource "azurerm_virtual_machine" "bastion2vm" {
  count = (var.environment_acronym == "dv" || var.environment_acronym == "pd")  ? 1 : 0

  name                  = local.bastion2vm_name
  location              = azurerm_resource_group.bstnhosts.location
  resource_group_name   = azurerm_resource_group.bstnhosts.name
  network_interface_ids = [element(azurerm_network_interface.bastion2nic.*.id, count.index)]
  ...
  storage_os_disk {
    name              = "${local.bastion2vm_name}_OSDisk"
    caching           = "ReadWrite"
  ...
  storage_data_disk {
    name              = "${local.bastion2vm_name}_DataDisk_1"
    managed_disk_type = "Standard_LRS"
    create_option     = "Empty"
    lun               = 0
    disk_size_gb      = "200"
  ...
  storage_data_disk {
    name              = "${local.bastion2vm_name}_DataDisk_2"
    managed_disk_type = "Standard_LRS"
    create_option     = "Empty"
    lun               = 1
    disk_size_gb      = "200"
  ...
  os_profile {
    computer_name  = local.bastion2vm_name
    admin_username = var.bastion2_admin_username
...
resource "azurerm_virtual_machine_extension" "bastion2vm_custom_ext_win_vm" {
  count = var.environment_acronym == "dv" ? 1 : 0

  name                       = "CustomScriptExtension"
  virtual_machine_id         = element(azurerm_virtual_machine.bastion2vm.*.id, count.index)
