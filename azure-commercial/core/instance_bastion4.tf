locals {
  bastion4vm_name = (var.environment_acronym == "dv" || var.environment_acronym == "pd") ? "UEMCBASTION4P1" : "VMW${upper(var.location_acronym)}BSTN4${upper(var.environment_acronym)}"
}

resource "azurerm_network_interface" "bastion4nic" {
  count = (var.environment_acronym == "dv" || var.environment_acronym == "pd")  ? 1 : 0
  
  name                = "${local.bastion4vm_name}-nic"
  resource_group_name = azurerm_resource_group.bstnhosts.name
  location            = azurerm_resource_group.bstnhosts.location

  ip_configuration {
    name                          = "${local.bastion4vm_name}-config"
    subnet_id                     = azurerm_subnet.bastion1.id
    private_ip_address_allocation = "Static"
    private_ip_address            = var.bastion4_nic_private_ip_address
    public_ip_address_id          = var.environment == "production" ? azurerm_public_ip.bastion4pip.id : ""
    #public_ip_address_id          = azurerm_public_ip.bastion4pip.id
  }

  tags = var.tags
}

resource "azurerm_virtual_machine" "bastion4vm" {
  count = (var.environment_acronym == "dv" || var.environment_acronym == "pd")  ? 1 : 0

  name                  = local.bastion4vm_name
  location              = azurerm_resource_group.bstnhosts.location
  resource_group_name   = azurerm_resource_group.bstnhosts.name
  network_interface_ids = [element(azurerm_network_interface.bastion4nic.*.id, count.index)]
  ...
  storage_os_disk {
    name              = "${local.bastion4vm_name}_OSDisk"
    caching           = "ReadWrite"
  ...
  storage_data_disk {
    name              = "${local.bastion4vm_name}_DataDisk_1"
    managed_disk_type = "Standard_LRS"
    create_option     = "Empty"
    lun               = 0
    disk_size_gb      = "200"
  ...
  storage_data_disk {
    name              = "${local.bastion4vm_name}_DataDisk_2"
    managed_disk_type = "Standard_LRS"
    create_option     = "Empty"
    lun               = 1
    disk_size_gb      = "200"
  ...
  os_profile {
    computer_name  = local.bastion4vm_name
    admin_username = var.bastion4_admin_username
...
resource "azurerm_virtual_machine_extension" "bastion4vm_custom_ext_win_vm" {
  count = var.environment_acronym == "dv" ? 1 : 0

  name                       = "CustomScriptExtension"
  virtual_machine_id         = element(azurerm_virtual_machine.bastion4vm.*.id, count.index)
