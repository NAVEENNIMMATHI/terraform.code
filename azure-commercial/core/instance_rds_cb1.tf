locals {
  rds1vm_name = (var.environment_acronym == "dv" || var.environment_acronym == "pd") ? "UEMCRDSCB1P1" : "VMW${upper(var.location_acronym)}RDSCB1${upper(var.environment_acronym)}"
}

resource "azurerm_network_interface" "rds1nic" {
  count = var.environment_acronym == "pd" ? 1 : 0

  name                = "${local.rds1vm_name}-nic"
  resource_group_name = azurerm_resource_group.rds.name
  location            = azurerm_resource_group.rds.location

  ip_configuration {
    name                          = "${local.rds1vm_name}-config"
    subnet_id                     = azurerm_subnet.rdcbl.id
    private_ip_address_allocation = "Static"
    private_ip_address            = var.rds1_nic_private_ip_address
  }

  tags = var.tags
}

resource "azurerm_virtual_machine" "rds1vm" {
  count = var.environment_acronym == "pd" ? 1 : 0

  name                  = local.rds1vm_name
  location              = azurerm_resource_group.rds.location
  resource_group_name   = azurerm_resource_group.rds.name
  network_interface_ids = [element(azurerm_network_interface.rds1nic.*.id, count.index)]
  ...
  storage_os_disk {
    name              = "${local.rds1vm_name}_OSDisk"
    caching           = "ReadWrite"
  ...
  storage_data_disk {
    name              = "${local.rds1vm_name}_DataDisk_1"
    managed_disk_type = "Standard_LRS"
    create_option     = "Empty"
    lun               = 0
    disk_size_gb      = "200"
  ...
  storage_data_disk {
    name              = "${local.rds1vm_name}_DataDisk_2"
    managed_disk_type = "Standard_LRS"
    create_option     = "Empty"
    lun               = 1
    disk_size_gb      = "200"
  ...
  os_profile {
    computer_name  = local.rds1vm_name
...
# resource "azurerm_virtual_machine_extension" "rds1vm_custom_ext_win_vm" {
#   count = var.environment_acronym == "dv" ? 1 : 0

#   name                       = "CustomScriptExtension"
#   virtual_machine_id         = element(azurerm_virtual_machine.rds1vm.*.id, count.index)
