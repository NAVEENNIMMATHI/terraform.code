module "spipaasukwstvm" {
  ...
  tags                         = var.tags
  subnet_id                    = azurerm_subnet.spipaaswst_subnet[var.vm_location].id
  count_value                  = var.count_value_spipaasukwst
...
resource "azurerm_network_interface" "network_interface_card" {
  for_each = var.wstn_vm

  name                              = "VM${upper(each.value.location_acronym)}SPIWK${each.value.count}${upper(var.environment_acronym)}-NIC${each.value.count}"
  ...
  ip_configuration {
    name                          = "VM${upper(each.value.location_acronym)}SPIWK${each.value.count}${upper(var.environment_acronym)}-NIC${each.value.count}-CONFIG"
...
resource "azurerm_virtual_machine" "virtual_machine" {
  for_each = var.wstn_vm
  name                             = "VM${upper(each.value.location_acronym)}SPIWK${each.value.count}${upper(var.environment_acronym)}"
  resource_group_name              = module.applicationrg[each.value.region].resource_group_name
  location                         = module.applicationrg[each.value.region].resource_group_location
...
storage_os_disk {
    name              = "VM${upper(each.value.location_acronym)}SPIWK${each.value.count}${upper(var.environment_acronym)}_OS_Disk"
    caching           = "ReadWrite"
  ...
  dynamic "storage_data_disk" {
    ...
    content {
      name              = "VM${upper(each.value.location_acronym)}SPIWK${each.value.count}${upper(var.environment_acronym)}_DataDisk_1"
  ...
  dynamic "storage_data_disk" {
    ...
    content {
      name              = "VM${upper(each.value.location_acronym)}SPIWK${each.value.count}${upper(var.environment_acronym)}_DataDisk_2"
      managed_disk_type = var.data_disk_1_override.managed_disk_type
      create_option     = var.data_disk_1_override.create_option
      lun               = 1
      disk_size_gb      = var.data_disk_1_override.disk_size_gb
  ...
  os_profile {
    computer_name =  "VM${upper(each.value.location_acronym)}SPIWK${each.value.count}${upper(var.environment_acronym)}" 
    admin_username = "VM${upper(each.value.location_acronym)}SPIWK${each.value.count}${upper(var.environment_acronym)}ADMIN" 
    admin_password = var.vm_admin_password
  }
  ...
  virtual_machine_id         = azurerm_virtual_machine.virtual_machine[each.key].id
