resource "azurerm_template_deployment" "ase_arm_bmp_deployment" {
  name                = "ase_arm_bmp_deployment"
...
DEPLOY
  ...
  parameters = {
    ...
    "subnet_name"                  = azurerm_subnet.bmpasesubnet.name
  ...
  depends_on = [azurerm_network_security_rule.Inbound_Allow_454-455_App_Service_Management_Addresses_asesubnetnsg]
}