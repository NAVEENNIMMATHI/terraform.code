...
resource "azurerm_application_gateway" "this" {
  ...
  http_listener {
    ...
    host_name                      = "${lower(var.application_name)}${lower(local.environment_type)}.kpmgcloudops.com"
  ...
  probe {
    ...
    host                = "${lower(var.application_name)}${lower(local.environment_type)}.kpmgcloudops.com"
    protocol            = "Https"
    path                = "/"
    interval            = "30"
    timeout             = "30"
  ...
  http_listener {
    ...
    host_name                      = "${lower(var.application_name)}rest${lower(local.environment_type)}.kpmgcloudops.com"
    frontend_ip_configuration_name = local.frontend_ip_configuration_name
    frontend_port_name             = local.frontend_port_name
    protocol                       = "Https"
    require_sni                    = "true"
  ...
  probe {
    ...
    host                = "${lower(var.application_name)}rest${lower(local.environment_type)}.kpmgcloudops.com"
    protocol            = "Https"
    path                = "/"
    interval            = "30"
    timeout             = "30"
