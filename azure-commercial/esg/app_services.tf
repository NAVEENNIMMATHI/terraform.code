resource "azurerm_app_service" "appservice_web" {
  name                = "${lower(var.application_name)}${lower(var.app_gateway_environment_acronym)}"
...
resource "azurerm_app_service" "appservice_rest" {
  name                = "${lower(var.application_name)}rest${lower(var.app_gateway_environment_acronym)}"
  location            = module.applicationrg.resource_group_location
  resource_group_name = module.applicationrg.resource_group_name
  app_service_plan_id = module.app_service_plan.app_service_plan_id
  https_only          = "true"
