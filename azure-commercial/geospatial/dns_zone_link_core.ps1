...
param (
    ...
    [string]$DNS_VNET_CORE,
    [Parameter(Mandatory=$True)]
    [string]$NODE_RGP
)

Write-Host $DNS_VNET_CORE
Write-Host $NODE_RGP
...
try {
    ...
    New-AzPrivateDnsVirtualNetworkLink -ResourceGroupName $NODE_RGP -ZoneName $ZONE_NAME -Name "adds1" -VirtualNetworkId $DNS_VNET_CORE
